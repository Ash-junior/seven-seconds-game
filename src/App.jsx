...
export default function SevenSecondChrono() {
  ...
  const [teams, setTeams] = useState(() => {
    ...
    if (!saved) return [
      { name: "Équipe A", avatar: base + "avatars/a.png", score: 0 },
      { name: "Équipe B", avatar: base + "avatars/b.png", score: 0 },
    ];
    ...
    return parsedTeams.map(team => ({
      ...team,
      avatar: team.avatar || base + `avatars/${team.name.toLowerCase().replace(/\s+/g, '')}.png`
    }));
  });
  ...
  const handleAddTeam = (e) => {
    ...
    const avatarUrl = teamAvatar 
      ...
      : base + `avatars/${teamName.toLowerCase().replace(/\s+/g, '')}.png`;
  ...
  const resetGame = () => {
    ...
    setTeams(teams.map(t => ({ 
      ...t, 
      score: 0,
      avatar: t.avatar || base + `avatars/${t.name.toLowerCase().replace(/\s+/g, '')}.png`
    })));
  };
